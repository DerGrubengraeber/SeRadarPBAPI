...
public class GrubenRadarAPI
{
    ...
    {
        if (program == null) throw new Exception("Pass `this` into the API constructor.");
        _pb = program.Me;
        var prop = _pb.GetProperty("GrubenRadarAPI");
        if (prop == null) return;
        var methods = prop.As < IReadOnlyDictionary < string,
            Delegate >> ().GetValue(_pb);
        if (methods == null) return;
        Delegate rawDelegate;
        if (!methods.TryGetValue("GetDetectedRadarTargets", out rawDelegate)) return;
    ...
    {
        if (_gtrt == null || pb == null) return new List < MyDetectedEntityInfo > ();
