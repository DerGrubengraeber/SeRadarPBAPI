...
namespace GrubenRadarApi
{
    public partial class RadarApiMod
    {
        ...
        void SetupApi()
        {
            PBInterface.AddMethod("GetDetectedRadarTargets", new Func < IMyProgrammableBlock, MemorySafeList < MyDetectedEntityInfo >> (API_GetDetectedRadarTargets));
        ...
        {
            var dict = obj as Dictionary < string,
                Delegate > ;
            if (dict != null)
            {
                NerdRadarAPI = dict;
        ...
        //The Actual Method(s)
        MemorySafeList < MyDetectedEntityInfo > API_GetDetectedRadarTargets(IMyProgrammableBlock pb)
        {
            ...
            MemorySafeList < MyDetectedEntityInfo > result = new MemorySafeList < MyDetectedEntityInfo > ();
        ...
        {
            if (entity == null) return new MyDetectedEntityInfo();
            ...
            if (entity.GetTopMostParent() != null && entity.GetTopMostParent().GetType() == typeof(MyCubeGrid))
            {
                ...
                BoundingBoxD worldAabb2 = new BoundingBoxD(topMostParent.PositionComp.WorldAABB.Min + posOffset, topMostParent.PositionComp.WorldAABB.Max + posOffset);
                Vector3 velOffset = GetRandomOffset(velError);
                Vector3 linearVelocity = topMostParent.Physics != null ? topMostParent.Physics.LinearVelocity + velOffset : velOffset;
        ...
        {
            if (maxOffset == 0.0 f) return Vector3.Zero;
            Vector3 offset = Vector3.One;
            int attempt = 0;
            while (offset.LengthSquared() > 1 f && attempt < 100)
            {
                offset.X = Random.GetRandomFloat(-1 f, 1 f);
                offset.Y = Random.GetRandomFloat(-1 f, 1 f);
                offset.Z = Random.GetRandomFloat(-1 f, 1 f);
                attempt++;
            }
            offset.Normalize();
